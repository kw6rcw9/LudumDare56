...
{
    public class Example11DictationSynthesis : MonoBehaviour
    {
        /// <summary>
		...
		/// Abort button
		///
		public Button _mButtonAbort = null;
        ...
        IEnumerator Start()
        {
            ...
            SpeechDetectionUtils.SetActive(false, _mTextSummary, _mTextDictation, _mButtonAbort, _mButtonSpeak);
            ...
            if (_mSpeechDetectionPlugin != null)
            {
                ...
                /*
				// start detection is not implemented by all browsers yet, commented out
				...
				*/
                ...
                /*
				// stop detection is not implemented by all browsers yet, commented out
                if (_mButtonStopDetection)
                {
				...
				*/
				...
				if (_mButtonAbort)
				{
					_mButtonAbort.gameObject.SetActive(true);
					_mButtonAbort.onClick.AddListener(() =>
					{
                ...
                if (_mButtonSpeak)
                {
                    _mButtonSpeak.gameObject.SetActive(true);
                    _mButtonSpeak.onClick.AddListener(() =>
                    {
                        // abort detection before speaking
                        _mSpeechDetectionPlugin.Abort();

                        // set some default text if not set
                        if (string.IsNullOrEmpty(_mSpeakText))
                        {
                            _mSpeakText = "Text to speech is great. Thumbs up!";
                        }
        ...
        bool HandleDetectionResult(DetectionResult detectionResult)
        {
            ...
            foreach (SpeechRecognitionResult result in results)
            {
                ...
                foreach (SpeechRecognitionAlternative alternative in alternatives)
                {
                    ...
                    // Record text to speak in case speak button is pressed
